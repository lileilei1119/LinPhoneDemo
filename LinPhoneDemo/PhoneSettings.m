...
//             pt->mime_type,pt->clock_rate];
            PLog(@"Codec %s/%i supported by core is not shown in iOS app config view.",
                 pt->mime_type,pt->clock_rate);
        }
    }
}
...
- (void)transformLinphoneCoreToKeys {
    ...
    {
        int port = lp_config_get_int(conf, LINPHONERC_APPLICATION_KEY, "port_preference", OUR_PORT.intValue);
...
//        [LinphoneLogger logc:LinphoneLoggerDebug format:"%@ IPV6", enable_ipv6?@"ENABLING":@"DISABLING"];
        PLog(@"%@ IPV6", enable_ipv6?@"ENABLING":@"DISABLING");
    ...
    if (username && [username length] >0 && domain && [domain length]>0) {
        ...
        if( ![PhoneSettings hasSipPrefix:proxyAddress] ) {
    ...
    *maxPort = -1;
    ...
    if([matches count] == 1) {
        ...
        if(!range) {
            ...
            *minPort = [PhoneSettings validPort:[[text substringWithRange:rangeMinPort] intValue]];
            *maxPort = *minPort;
            return TRUE;
        } else {
            NSRange rangeMinPort = [match rangeAtIndex:1];
            ...
            *maxPort = [PhoneSettings validPort:[[text substringWithRange:rangeMaxPort] intValue]];
            if(*minPort > *maxPort) {
                *minPort = *maxPort;
            }
            return TRUE;
...
//         NSLog(@"lileilei==%s===%i",pt->mime_type,[self boolForKey: pref]);
    }
    ...
    // Audio & Video Port
    {
        ...
        int minPort, maxPort;
        [PhoneSettings parsePortRange:audio_port_preference minPort:&minPort maxPort:&maxPort];
    ...
    }
    {
        ...
        int minPort, maxPort;
        [PhoneSettings parsePortRange:video_port_preference minPort:&minPort maxPort:&maxPort];
...
//                [LinphoneLogger logc:LinphoneLoggerError format:"Unexpected tunnel mode [%s]",[lTunnelPrefMode cStringUsingEncoding:[NSString defaultCStringEncoding]]];
                PLog(@"Unexpected tunnel mode [%s]",[lTunnelPrefMode cStringUsingEncoding:[NSString defaultCStringEncoding]]);
            }
        }
